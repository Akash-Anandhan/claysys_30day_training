...
namespace ShoppingCartApp.Services
{
    public partial class AdminService
    {
        ...
        public async Task<AdminDashboardDto> GetDashboardAsync(string dateRange = "This Month")
        {
            ...
            // Calculate AOV Trend (group by date)
            var aovTrend = currentOrders
                .GroupBy(o => o.OrderDate.ToString("MMM dd"))
                .OrderBy(g => g.Min(o => o.OrderDate))
                .Select(g => new {
                    Label = g.Key,
            ...
            // Calculate User Registration Trend (group by registration date)
            var usersList = await _context.Users
                .Where(u => u.CreatedAt >= startDate)
                .Select(u => new { u.CreatedAt })
                .ToListAsync();

            var userRegTrend = usersList
                .GroupBy(u => u.CreatedAt.ToString("MMM dd"))
                .OrderBy(g => g.Key)
                .Select(g => new {
                    Label = g.Key,
                    Count = g.Count()
            ...
            var userRegLabels = userRegTrend.Select(x => x.Label).ToList();
            var userRegCounts = userRegTrend.Select(x => x.Count).ToList();

            if (!userRegLabels.Any())
            {
                userRegLabels = new List<string> { "No Data" };
                userRegCounts = new List<int> { 0 };
            }
            ...
            return new AdminDashboardDto
            {
                ...
                UserRegistrationLabels = userRegLabels,
                UserRegistrationCounts = userRegCounts,
        ...
        // ── Populate Payment Methods for Existing Orders ──
        public async Task<int> PopulatePaymentMethodsAsync()
        {
            var paymentMethods = new[] { "Credit Card", "Debit Card", "PayPal", "Bank Transfer", "Cash on Delivery" };
            var random = new Random();

            // Get all orders without payment method
            var ordersWithoutPayment = await _context.Orders
                .Where(o => string.IsNullOrWhiteSpace(o.PaymentMethod))
                .ToListAsync();

            foreach (var order in ordersWithoutPayment)
            {
                order.PaymentMethod = paymentMethods[random.Next(paymentMethods.Length)];
            }

            await _context.SaveChangesAsync();
            return ordersWithoutPayment.Count;
        }
    }
}
